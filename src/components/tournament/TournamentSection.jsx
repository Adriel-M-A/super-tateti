...
const TournamentSection = ({ gameType, onTournamentChange }) => {
  ...
  const { players, updatePlayer, getVisiblePlayers, getTakenResources } =
    usePlayerSetup(3, 6, null);
  const { competitiveMode, turnTime, competitiveSelectorProps } =
    useCompetitiveSetup(false, 0);
  ...
  const stableChange = useCallback(
    (config) => onTournamentChange(config),
    [onTournamentChange],
  );
  ...
  useEffect(() => {
    ...
    if (format === "elimination") {
      shuffledPlayersRef.current = [...currentPlayers].sort(
        () => Math.random() - 0.5,
      );
    } else {
  ...
  const handleNumPlayersChange = (n) => {
    ...
    if (n > numPlayers) {
      ...
      if (taken.takenColors.includes(newPlayer.color)) {
        const free = COLOR_OPTIONS.find(
          (o) => !taken.takenColors.includes(o.hex),
        );
  ...
  const btnBase =
    "p-3 rounded-xl text-left transition-all border text-xs font-black uppercase tracking-tight";
  const btnActive = "border-yellow-500/60 bg-yellow-500/10 text-yellow-400";
  const btnInactive =
    "border-board-border bg-page-text/5 text-slate-400 hover:text-page-text";
  ...
  return (
    ...
    >
      ...
      <button
        ...
        className="w-full flex items-center justify-between p-5 gap-4 text-left hover:bg-page-text/5 transition-colors"
      >
        <div className="flex items-center gap-3">
          <div
            className={`p-2 rounded-xl transition-colors ${isEnabled ? "bg-yellow-500/20 text-yellow-400" : "bg-page-text/10 text-slate-400"}`}
          >
            <Trophy size={18} />
          </div>
          <div>
            <p className="font-black uppercase tracking-tighter text-sm text-page-text">
              Modo Torneo
            </p>
        ...
        {/* Toggle switch */}
        <div
          className={`w-11 h-6 rounded-full transition-all duration-300 relative shrink-0 ${isEnabled ? "bg-yellow-500" : "bg-page-text/20"}`}
        >
          <div
            className={`absolute top-0.5 w-5 h-5 rounded-full bg-white shadow transition-all duration-300 ${isEnabled ? "left-5" : "left-0.5"}`}
          />
        </div>
      </button>
            ...
            <div className="grid grid-cols-2 gap-2">
              {formats.map((f) => (
                ...
                >
                  ...
                  <p className="text-[9px] opacity-60 mt-0.5 normal-case font-normal">
                    {f.desc}
                  </p>
                </button>
              ))}
